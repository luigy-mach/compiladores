...
void class_analizador_lexico::obtener_vector_tokens(vector< pair<string,string> >& vec){
  ...
  for(it=vec.begin(); it!=vec.end();){
    if(it->second==STR_ESPACIO_EN_BLANCO ||
    ...
    else if(it->second==STR_DOUBLE ||
            it->second==STR_ENTERO ||
            it->second==STR_HEXADECIMAL ||
            it->second==STR_OCTAL ){
      string out="numero";
    ...
    else if(it->second==STR_CHAR ||
       it->second==STR_STRING || it->second==STR_STRING_SIN_DEPURAR ){
        string out="id";
        if(_mytabla->buscar(it->first,out)){
      ...
      string out="____ERROR_EN_TABLA_GRAVE";
      if(_mytabla->buscar(it->first,out)){
          // cout<<"TOKEN: "<<it->second<<endl;
          it->second=out;
      }else{
...
void class_analizador_lexico::cargar_automata(){
  ...
  _myautomata->insertar_estado("q3",3,STR_STRING);
  _myautomata->insertar_estado("q4",0,STR_SIN_TOKEN);
  _myautomata->insertar_estado("q5",5,STR_COMILLA_SIMPLE);//"CHAR_INCOMPLETO"
  _myautomata->insertar_estado("q6",6,STR_CHAR);
  ...
  _myautomata->insertar_estado("q13",13,STR_STRING_SIN_DEPURAR); //id
  _myautomata->insertar_estado("q14",14,STR_ENTERO);//solo un cero
  _myautomata->insertar_estado("q15",0,STR_SIN_TOKEN);
  _myautomata->insertar_estado("q16",16,STR_HEXADECIMAL);
  _myautomata->insertar_estado("q17",0,STR_SIN_TOKEN);
  _myautomata->insertar_estado("q18",18,STR_ENTERO);//acepta puro ceros
  _myautomata->insertar_estado("q19",0,STR_SIN_TOKEN);
  _myautomata->insertar_estado("q20",20,STR_DOUBLE);
  ...
  _myautomata->insertar_estado("q24",24,STR_OCTAL);
